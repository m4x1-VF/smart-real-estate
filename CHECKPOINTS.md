...
- [ ] No hay dependencias externas en `package.json` fuera de las declaradas
...
- [ ] Los tests usan directorios temporales reales (`os.tmpdir()` +
      `fs.mkdtempSync`), no mocks de `fs`.
- [ ] `pnpm test:run` muestra > 0 tests y todos verdes.

## C5 — La sesión se cerró bien

- [ ] No hay archivos sin trackear sospechosos (`*.tmp`, `node_modules/.cache`,
