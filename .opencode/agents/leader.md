...
### Caso E — el alcance de una feature cerrada (`done`) cambia

Si el humano pide modificar, ampliar o ajustar una feature que ya está
`done`, **NO crees una feature nueva**. La regla es:

1. Reabrir la feature original: cambia su status de `done` a `in_progress`
   (o `spec_ready` si el cambio de alcance invalida el spec existente).
2. Si el cambio es mayor (nuevos requirements, diseño o tasks que
   contradicen el spec), lanza primero **1 `spec_author`** para actualizar
   `specs/<name>/` antes de pasar a implementación.
3. Si el cambio es menor (ajuste de texto, fix puntual, refactor pequeño),
   actualiza el spec con un apéndice `## Changelog` y lanza directamente
   al `implementer`.
4. Nunca dupliques features en `feature_list.json` con nombres derivados
   del tipo `<name>-v2`, `<name>-extended`, etc.
